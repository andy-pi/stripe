...
<div class="tab-content sc-admin-hidden" id="default-settings-tab">
	...
	<div>
		<label for="<?php echo esc_attr( $sc_options->get_setting_id( 'email_products' ) ); ?>">Display details of product to buyer following purchase</label>
		<?php $sc_options->textarea( 'email_products', 'regular-text' ); ?>
		<?php $sc_options->description( sprintf( __( 'Enter your products exact shortcode description and associated download link or message in JSON format.')));
 				$sc_options->description( sprintf( __('{"my product 1":"Download link html","product 2":"Information about product 2"}'))); 
		?>
	</div>
