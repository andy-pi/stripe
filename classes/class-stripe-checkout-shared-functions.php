				...
				if ( isset( $_GET['charge'] ) && ! isset( $_GET['charge_failed'] ) ) {
					...
					if ( null === $sc_options->get_setting_value( 'disable_success_message' ) ) {
						...
						if ( ! empty( $charge_response->description ) ) {
							...
							$product_shortcode_desc=$charge_response->description;
							$products_list=$sc_options->get_setting_value('email_products');
							$products_list=json_decode($products_list,true);
	
							if( isset( $products_list[$product_shortcode_desc] ) ){
   								$product_url = $products_list[$product_shortcode_desc];
        						$html .= $product_url . '<br/>' . "\n";
    						}
